...
query="SELECT slot, dept_no FROM entry"
mycursor.execute(query)
res=mycursor.fetchall()
query="INSERT INTO parkslots VALUES(%s, %s)"
for i in res:
    tup1=i
...
def search(numplate):
    query="SELECT * FROM entry WHERE numberplate= %s"
    tup1=(numplate,)
    mycursor.execute(query,tup1)
    res=mycursor.fetchall()
    if not res:
        query="SELECT dept_no FROM empdetails WHERE numplate = %s"
        mycursor.execute(query, tup1)                                                                         #searching numplate in database
        res=mycursor.fetchall()
        if res:
            dept=res[0]
            dept=dept[0]
            ...
            query="SELECT slots FROM parkslots WHERE department = %s limit 1"
            mycursor.execute(query,tup2)
            res1=mycursor.fetchall()
            slot=res1[0]
            slot=slot[0]
            query="DELETE FROM parkslots WHERE slots= %s" 
            tup2=(slot,)
            mycursor.execute(query,tup2)
            query="INSERT INTO entry (Numberplate, Slot, Dept_no) VALUES(%s, %s, %s)"                          #inserting detail in table
            tup3=(numplate,slot,dept)
            mycursor.execute(query, tup3)
            mydb.commit()
